...
{
    /* Create window */
    window_name_ = "Signaling Traffic For Safe Crossing";
...
//    resize(images_.at(2), images_.at(2), cv::Size(images_.at(2).cols / 4, images_.at(2).rows / 4));
    resize(images_.at(2), images_.at(2), cv::Size(images_.at(2).cols/2, images_.at(2).rows/2));
    imshow(window_name_, images_.at(2));
//    imwrite( "./images/Gray_Image"+std::to_string(plot_count++)+".jpg", images_.at(2));
    cv::waitKey(33);
}
